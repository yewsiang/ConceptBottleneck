...
def simulate_group_intervention(mode, replace_val, use_not_visible, min_uncertainty, n_replace, use_relu, use_sigmoid, n_trials=1):
    assert len(instance_attr_labels) == len(b_attr_labels), 'len(instance_attr_labels): %d, len(b_attr_labels): %d' % (len(instance_attr_labels), len(b_attr_labels))
    assert len(uncertainty_attr_labels) == len(b_attr_labels), 'len(uncertainty_attr_labels): %d, len(b_attr_labels): %d' % (len(uncertainty_attr_labels), len(b_attr_labels))
    ...
    for _ in range(n_trials):
        ...
        if mode == 'random':
            ...
            replace_fn = lambda attr_preds, attr_preds_sigmoid, attr_labels, img_id, n_replace, replace_cached: replace_entropy_adaptive(attr_preds, attr_preds_sigmoid, attr_labels, img_id, n_replace, replace_cached)
            ...
            def replace_entropy_adaptive(attr_preds, attr_preds_sigmoid, attr_labels, img_id, n_replace, replace_cached):
    ...
    return max(all_class_acc), attr_replace_idx, all_attr_ids, b_class_outputs_new
...
def parse_arguments(parser=None):
    ...
    parser.add_argument('-use_attr', help='whether to use attributes (FOR COTRAINING ARCHITECTURE ONLY)',
                        action='store_true')
    ...
    parser.add_argument('-data_dir', default='class_attr_data_10', help='directory to the data used for evaluation')
    ...
    parser.add_argument('-attribute_group', action='store_true',
                        help='file listing the (trained) model directory for each attribute group')
    parser.add_argument('-feature_group_results', help='whether to print out performance of individual atttributes',
                        action='store_true')
    ...
    args.use_invisible = True
    args.use_attr = True
    args.n_groups = 28
    args.class_level = True
    args.mode = 'random'
    args.n_trials = 5
...
def run(args):
    ...
    curr_group_idx = 0
    with open('datasets/CUB_200_2011/attributes/attributes.txt', 'r') as f:
    ...
    b_class_labels, b_topk_class_outputs, b_attr_labels, b_attr_outputs, b_attr_outputs_sigmoid, b_wrong_idx, b_attr_outputs2 = eval(
        args)
    ...
    for n_replace in list(range(args.n_groups + 1)):
        if 'random' not in args.mode:
            N_TRIALS = 1
        acc, attr_replace_idx, all_attr_ids, new_preds = simulate_group_intervention(args.mode, REPLACE_VAL,
                                                                                     args.use_invisible,
                                                                                     MIN_UNCERTAINTY_GAP, n_replace,
                                                                                     args.use_relu, args.use_sigmoid,
                                                                                     n_trials=N_TRIALS)
        print(n_replace, acc)
